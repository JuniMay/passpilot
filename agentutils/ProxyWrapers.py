...
from agentutils import URLUtils
...
class ProxyWrapper():
    ...
    _base_config = {
        "force_cookies": {
            # Useful to force cookies on the wire. This can be done to avoid sending out persistent cookies (even though they will be present in the browser)
        ...
        "redirection_mode": {
            # Extract redirection flow from a given starting URL (e.g. http://example.com -> https://example.com -> https://www.example.com). Headers included
            "enabled": False,
        ...
        "spoofing_mode": {  # Spoof any header. If the header exists, it will be replaced with the spoofed one
            "enabled": False,
            "headers": {},
            # Dictionary of the form: {"origin" : "abc.com", "user-agent" : "whatever"}. These will be spoofed for all requests
            "domain_headers": []
            # Domain-specific rules. Similar as the `force_cookies` mode rules. E.g. {"domain" : "abc.com", "headers" : {"x-header" : "blah"}"}
        }
    }
    ...
    def _get_free_port(cls):
        ...
        raise Exception(
            "Not available ports in range [%s,%s)" % (ProxyWrapper._port_range[0], ProxyWrapper._port_range[-1]))

    def __init__(self, port=None, strip_media=False, mitm_script=None, tor={"enabled": False},
                 custom_proxy={"enabled": False}):
        self._proxy_intercomm_file = os.path.join(ProxyWrapper._abs_path, ProxyWrapper._proxy_intercomm_dir,
                                                  "xdriver_proxy-%s" % str(
                                                      uuid4()))  # Unique ID used for inter-process comm between ProxyWrapper and the mitmproxy subprocess
        ...
        if custom_proxy["enabled"]:  # Route everything through user defined proxy
            proc = subprocess.Popen([ProxyWrapper._mitm_bin, "-s", "%s %s %s" % (
            ProxyWrapper._mitm_script if not mitm_script else mitm_script, self._proxy_intercomm_file,
            int(strip_media)), "-p", str(self._proxy_port), "--insecure", "--quiet", "-U", "%s://%s:%s" % (
                                     custom_proxy["scheme"], custom_proxy["host"], custom_proxy["port"])])
        else:
            proc = subprocess.Popen([ProxyWrapper._mitm_bin, "-s", "%s %s %s" % (
            ProxyWrapper._mitm_script if not mitm_script else mitm_script, self._proxy_intercomm_file,
            int(strip_media)), "-p", str(self._proxy_port), "--insecure", "--quiet"])

        sleep(2)
        retcode = proc.poll()  # Just to make sure
        if retcode:  # No exit code, no prob
            raise Exception(f"Proxy could not boot in {ProxyWrapper._caller_prefix}.")
    ...
    def load_config(self):
        while True:
            try:
                with open(self._proxy_intercomm_file, 'rb') as fp:
                    return cp.load(fp)
            except EOFError:
                continue
    ...
    def redirection_mode(self, url):
        ...
        self._proxy_config["redirection_mode"]["url"] = url if url.startswith("http://") or url.startswith(
            "https://") else "%s%s" % ("http://", url)
